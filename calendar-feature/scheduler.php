        ...
        <div class="form-container">
            ...
            <form action="scheduler_send_invite.php" method="post" id="event-form">
                ...
                <div class="form-group">
                    ...
                    // Fetch events from the database
                    $query = "SELECT CONTACT_ID, CONTACT_EMAIL, CONTACT_USER_ID FROM CONTACTS WHERE CURRENT_USER_ID='$user_id'";
                    $result = $mysqli->query($query);

                    // Check if the query executed successfully
                    if ($result && $result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            echo "<option value='{$row['CONTACT_USER_ID']}'>{$row['CONTACT_EMAIL']}</option>";
                        }
                    } else {
